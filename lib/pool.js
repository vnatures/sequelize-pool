...
class Pool {
  ...
  _dispense() {
    ...
    while (this._availableObjects.length > 0) {
      ...
      const callbackWithTimeout = this._pendingAcquires.shift();
      return callbackWithTimeout.callback(null, resourceWithTimeout.resource);
    }
  ...
  _createResource() {
    ...
    this._factory.create((error = null, object = null) => {
      ...
      if (error) {
        this._count -= 1;
        if (this._count < 0) this._count = 0;
        if (callbackWithTimeout && callbackWithTimeout.callback) {
          callbackWithTimeout.callback(error, object);
        }
        process.nextTick(() => {
          this._dispense();
        });
   ...
   *
   * callback will be called with an timeout error if `factory.create` didn't respond
   * back within specific `acquireTimeoutMillis`
   *
   * @param {Function} callback
   *   Node style callback function to be called after the acquire is successful.
  ...
  acquire(callback) {
    ...
    const callbackWithTimeout = {
      callback,
      timeout: Date.now() + this._factory.acquireTimeoutMillis
    };

    this._pendingAcquires.push(callbackWithTimeout);
    this._dispense();
