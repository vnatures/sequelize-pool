...
Pool.prototype._log = function log(str, level) {
  if (typeof this._factory.log === "function") {
    ...
    console.log(
      level.toUpperCase() + " pool " + this._factory.name + " - " + str
    );
  }
};
...
Pool.prototype.destroy = function destroy(obj, cb) {
  ...
  this._availableObjects = this._availableObjects.filter(function(
    objWithTimeout
  ) {
 ...
 */
Pool.prototype.pooled = function pooled(decorated) {
  var self = this;
  return function() {
    ...
    self.acquire(function(err, client) {
      ...
      var args = [client].concat(
        Array.prototype.slice.call(
          callerArgs,
          0,
          callerHasCallback ? -1 : undefined
        )
      );
      args.push(function() {
        self.release(client);
